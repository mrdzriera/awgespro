...
$consulta5="SELECT * FROM proyectos_tiene_eva_def,proyectos WHERE proyectos.id=proyectos_tiene_eva_def.id_proyecto
AND proyectos.id='".$id_proyecto."' AND
...
$respuesta5=mysql_query($consulta5);

if(mysql_num_rows($r)==0){
	?>
<script language="javascript">
$(document).ready(function(){
sweetAlert("Disculpe", "No se le puede realizar entrega de documentos al proyecto porque no tiene tutor asignado", "error");
});
function redireccionar(){
window.location="listado_nosolventes.php?solvente=no";
}
setTimeout('redireccionar()',3000);
</script>
	<?php
} else if((mysql_num_rows($respuesta1)==0)or(mysql_num_rows($respuesta2)==0)or(mysql_num_rows($respuesta3)==0)
or(mysql_num_rows($respuesta4)==0)or(mysql_num_rows($respuesta5)==0)or(mysql_num_rows($bus)==0)or
(mysql_num_rows($bus2)==0)or(mysql_num_rows($bus3)==0)){
		?>
<script language="javascript">
$(document).ready(function(){
sweetAlert("Disculpe", "Este proyecto no ha cumplido con las presentaciones y/o evaluaciones y/o fecha de socializacion y/o evaluacion definitiva,por favor registrelos", "error");
});
function redireccionar(){
window.location="listado_nosolventes.php?solvente=no";
}
...
$sql1="UPDATE proyectos_tiene_documento SET portada='".$portada."',acta_coord='".$acta_coord."',
...
$var="INSERT INTO historial VALUES ('null',".$_SESSION['id_usu'].",  'EL SECRETARIO QUE REGISTRO LA ENTREGA DE DOCUMENTOS TIENE ESTE ID:".$_SESSION['id_usu']. "','".date("Y-m-d")."','".date("h:i")."')";
//echo $sql;
mysql_query($var);
...
<script language="javascript">
$(document).ready(function(){
swal("Bien!", "Los documentos han sido registrados exitosamente", "success");
});
